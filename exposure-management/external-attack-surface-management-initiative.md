...
1. Select the footprint you want to use from the list of available pre-built footprints and choose **Connect**.
...
1. In up to 1 hour, the initiative is populated with high-level metrics and scores from the selected footprint.
...
1. Create a Resource Group with the appropriate subscription and region.
1. Deploy an MDEASM Resource within that group; see [Create a Defender EASM Azure resource](/azure/external-attack-surface-management/deploying-the-defender-easm-azure-resource). Each new resource will automatically get a free 30 day trial.
...
1. You can alternately create a custom discovery group by providing:
...
For more information, see [Discover your attack surface](/azure/external-attack-surface-management/discovering-your-attack-surface)
...
You can also explore the data integrated from EASM using the [attack surface map](enterprise-exposure-map.md) to uncover insights related to your attack surface. You can search for various assets such as IP addresses, domains, hosts, and more, and review the findings on these assets.
