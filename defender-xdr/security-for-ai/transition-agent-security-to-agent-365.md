...
1. **Migrate alert workflows** - Move any monitoring or response workflows that consume legacy alerts to their replacements by July 1, 2026:
   - Audit and block data from real-time protection rules moves to the `BehaviorInfo` table in Advanced Hunting. Build custom detections, hunting queries, and downstream automation against [behaviors](../advanced-hunting-behaviorinfo-table.md) in place of the legacy alerts.
