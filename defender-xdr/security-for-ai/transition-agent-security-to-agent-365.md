...
- **Real-time protection for Microsoft Copilot Studio**: Alerts based on near-real detections and blocked actions actions generated by the legacy real-time protection rules will be deprecated and replaced by equivalent Advanced Hunting [behaviors](../advanced-hunting-behaviorinfo-table.md). Behaviors are an Advanced Hunting signal type that record the same near-real detections and blocked actions as queryable telemetry, so you can build custom detections, hunting queries, and downstream automation against them in place of the legacy alerts.
- **Microsoft Agent 365 real-time protection**: Alerts for audits and blocks generated by legacy real-time protection rules will be deprecated and replaced by equivalent Advanced Hunting [behaviors](../advanced-hunting-behaviorinfo-table.md). Behaviors are an Advanced Hunting signal type that record audit and block events as queryable telemetry, so you can build custom detections, hunting queries, and downstream automation.
