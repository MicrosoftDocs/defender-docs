...
- (GA) [Built-in alert tuning rules](investigate-alerts.md#built-in-alert-tuning-rules) are now generally available across all Microsoft Defender XDR services, including Defender for Office 365, Defender for Identity, Defender for Cloud Apps, and Microsoft Entra ID Protection. Built-in alert tuning rules help reduce reporting noise from common benign activity without affecting Automated Investigation and Response (AIR) investigations and email notifications.
