...
- (Preview) **Identity scenarios in the hunting graph**: The [hunting graph](advanced-hunting-graph.md) now includes predefined identity-focused scenarios that visualize identity attack paths and exposure risks as interactive graphs. Discover lateral movement paths, privilege escalation routes, and credential-access risks from the hunting graph or directly from the Domain and User pages in Microsoft Defender for Identity. [Learn more](hunting-graph-identity.md)
