...
- (Preview) The [`AgentsInfo`](advanced-hunting-agentsinfo-table.md) table in advanced hunting is now available in preview. This new table replaces the [`AIAgentsInfo`](advanced-hunting-aiagentsinfo-table.md) table with a unified schema that supports agent inventory and governance for all agent types, including Copilot Studio, Azure AI Foundry, Microsoft 365 Copilot, third-party, and endpoint-discovered agents. The legacy `AIAgentsInfo` table remains available until July 1, 2026. Update your queries to use `AgentsInfo` before this date. For more information, see [Advanced hunting schema - Naming changes](advanced-hunting-schema-changes.md).
