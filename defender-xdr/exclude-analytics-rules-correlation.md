...
This article explains how to include or exclude analytics rules from the Microsoft Defender XDR correlation engine. Including or excluding analytics rules from correlation helps organizations migrating from Microsoft Sentinel maintain predictable incident behavior and ensure compatibility with existing automation workflows.


## Overview

Microsoft Defender groups multiple alerts and incidents into unified *attack stories*. While this capability provides powerful security insights, it can lead to unexpected behavior for organizations migrating from Microsoft Sentinel, where incidents are static and determined solely by analytics rule configurations.
...
For more information about how correlation works in Microsoft Defender, see [Alert correlation and incident merging in the Microsoft Defender portal](alerts-incidents-correlation.md).
...
Set the default incident correlation behavior for alerts and incidents generated by analytics rules across your tenant. The tenant's default incident correlation setting determines whether Microsoft Defender includes in its correlation engine all analytics rules that don't have their incident correlation behavior explicitly configured.
...
To set a rule's correlation state manually, add the appropriate tag to the beginning of the rule description.
...
For more information about using the Microsoft Defender API, see [Microsoft Defender APIs overview](api-overview.md).

<a name="important-considerations"></a>
## Important considerations for analytics rule correlation settings
