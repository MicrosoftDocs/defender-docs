...
|**Defender Chat experience (preview)**|An open-prompt chat assistant that enables natural language conversations directly within the Microsoft Defender portal. It provides page-context awareness, allowing analysts to ask follow-up questions about the current incident, alert, device, or entity without providing IDs or names. The chat maintains full conversation history, can present step-by-step investigation plans for complex requests, and supports export of responses for further analysis. <br/><br/> For more information, see [Microsoft Security Copilot and Chat in Microsoft Defender](security-copilot-in-microsoft-365-defender.md).|
