...
> [!NOTE]
> While the incident and alert details show historical data from the inception of the incident, the **Activities** tab shows a snapshot of the current status. For more information, see [Track the action status in the Activities tab](autoad-results.md#track-the-action-status-in-the-activities-tab).
