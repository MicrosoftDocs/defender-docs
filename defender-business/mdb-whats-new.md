...
- **Attack surface reduction capabilities are rolling out**. [Attack surface reduction capabilities in Defender for Business](mdb-asr.md) include attack surface reduction (ASR) rules and a new attack surface reduction rules report. ASR rules target certain behaviors that are considered risky because they're commonly abused by attackers through malware. In the Microsoft Defender portal (<https://security.microsoft.com>), you can now view a report showing detections and configuration information for ASR rules. In the navigation pane, choose **Reports**, and under **Endpoints**, choose **Attack surface reduction rules**.
