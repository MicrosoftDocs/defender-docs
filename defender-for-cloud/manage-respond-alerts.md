...
1. (Optional) Filter the alerts list with any of the relevant filters. You can add extra filters with the **Add filter** option.
    ...
    The alerts list updates according to the filters you select. For example, you might you want to address security alerts that occurred in the last 24 hours because you're investigating a potential breach in the system.
...
1. Select **View full details**.
    ...
    The right pane includes the **Alert details** tab containing further details of the alert to help you investigate the alert: IP addresses, files, processes, and more.
    ...
    For more information about the alert, contact the resource owner to verify whether the detected activity is a false positive. You can also, investigate the raw logs generated by the attacked resource.
...
1. Use the **Change status** options to set the desired status.
    ...
    The alerts shown on the Security alerts page have their status changed to the selected value.
...
To learn about alert types, see [Security alerts - a reference guide](alerts-reference.md).

For information about how Defender for Cloud detects and responds to threats, see [How Microsoft Defender for Cloud detects and responds to threats](alerts-overview.md).
...
> [!NOTE]
> Remediating the agent-based alert will not remediate the corresponding agentless alert until the next scan is completed.
