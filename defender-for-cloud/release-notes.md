...
| -------- | -------- | -------- |
| August 21, 2026 | Upcoming change | [Update to CVE details data consumption in Azure Resource Graph](#update-to-cve-details-data-consumption-in-azure-resource-graph) |
...
Microsoft Defender for Cloud is updating how vulnerability assessment CVE details are consumed in Azure Resource Graph. This isn't a retirement of CVE details. CVE information remains available, but detailed CVE properties are consumed through the `microsoft.security/cvedetails` resource type to improve query performance, scalability, and support for larger data sets.

Customers who use existing API or Azure Resource Graph queries that read CVE details from vulnerability assessment records should update those queries to consume CVE details from the updated Azure Resource Graph resource. Queries that aren't updated might stop returning the expected detailed CVE properties after the change.
