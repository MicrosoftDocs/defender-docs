---
title: Verify Defender for SQL Servers on Machines protection for SQL VMs in government clouds
...
1. If the `ProtectionStatusLastUpdate` field doesn't show a date within the last day, the machine might not be protected. To confirm, [verify the protection on a single SQL server VM](#verify-protection-on-a-single-sql-server-vm) by checking its **Protection status** under **Security** > **Defender for Cloud** in the Azure portal.
...
If databases aren't protected, follow the instructions in [Troubleshoot SQL machine protection issues](troubleshoot-sql-machines-guide-gov.md) to remediate the issues.
