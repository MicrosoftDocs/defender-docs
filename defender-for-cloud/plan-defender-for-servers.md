...
| **Take advantage of free data ingestion** | To take advantage of 500 MB of free daily ingestion per node for eligible security data, enable Defender for Servers Plan 2 on the Log Analytics workspace to which the machines report. Data must be collected through a supported method, such as Azure Monitor Agent (AMA). Creating a data collection rule alone doesn't enable the benefit. [Learn more](data-ingestion-benefit.md). | Free daily ingestion is configured for supported data types. |
