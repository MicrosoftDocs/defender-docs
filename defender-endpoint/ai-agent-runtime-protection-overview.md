...
Microsoft Defender for Endpoint provides runtime protection for local AI agents. When enabled, Defender monitors activity in the agentic loop and blocks inline when prompt injection attacks are identified. Runtime protection generates alerts in the Microsoft Defender portal and correlates them into incidents for investigation.

This article explains how runtime protection works, describes the two protection methods (agent hooks and network inspection), and covers enforcement modes and investigation workflows.
...
Defender Antivirus scans these payloads for prompt injection attacks and malicious instructions before risky actions are allowed to continue.
...
When either protection method detects a threat, Microsoft Defender surfaces an alert on the device timeline ([endpoint detection and response](/defender-endpoint/overview-endpoint-detection-response)) and correlates related alerts into incidents. Security teams can investigate these detections by using Microsoft Defender XDR workflows, including timeline review ([endpoint detection and response](/defender-endpoint/overview-endpoint-detection-response)), alert and entity correlation, and response actions.
...
- **Investigation is built into your workflow**: When threats are detected, alerts appear in the Microsoft Defender portal as part of the device timeline ([endpoint detection and response](/defender-endpoint/overview-endpoint-detection-response)) and are correlated into incidents. Your security team uses the same investigation workflows they're familiar with for other endpoint detections. For more information, see [View and investigate alerts in Microsoft Defender](/defender-endpoint/investigate-alerts).
...
> [!NOTE]
> Network inspection can protect additional agents that communicate over the network, but Microsoft Defender documents only a closed list of known supported agents. Network inspection doesn't support agents that use certificate pinning or HTTP/3.
