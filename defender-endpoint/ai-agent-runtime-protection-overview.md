...
- **Agent event scanning** (also known as **agent hooks**): Uses lifecycle events exposed by supported agent frameworks to inspect prompts, tool calls, and tool responses at defined points in the agentic loop.
- **Network inspection**: Inspects relevant traffic between the agent and large language model (LLM) endpoints when agent event scanning isn't available.

These methods work independently. Agent event scanning is the primary option for the closed list of known supported agents, while network inspection expands coverage to additional agents. For guidance on which method to use, see [Decide which method to use](configure-ai-agent-runtime-protection.md#decide-which-method-to-use).

## Performance and privacy

Runtime protection is designed for minimal performance impact. When agent event scanning is used, scanning occurs only at defined lifecycle events in the agentic loop, with no continuous monitoring of agent processes. Network inspection adds overhead only for agents that don't support hooks. No prompt content or user data is stored by Defender. Only threat metadata is retained for alert generation.
