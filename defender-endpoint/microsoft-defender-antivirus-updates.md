...
- Platform updates can be temporarily postponed if other protection features (such as [Endpoint DLP](/Microsoft-365/compliance/endpoint-dlp-getting-started.md) or [Device Control](device-control-report)) are actively monitoring running processes. Platform updates are retried after a reboot or when all monitored services are stopped.
