...
> [!NOTE]
> When a user undergoes a Microsoft account password reset, it is expected that the associated mobile device will lose corporate access if the user does not reauthenticate. Once the user reauthenticates, the device will resume communication with the security portal. This policy ensures the security of the user's account and helps prevent unauthorized access.
