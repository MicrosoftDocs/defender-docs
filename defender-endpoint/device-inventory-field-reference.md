...
|-----|-----------------|----------|---------------------------|
| **All devices** | Every device visible in device inventory, including devices from integrated sources when available | All of the following | Automatic; this tab aggregates all other tabs |
...
| **Network devices** | Routers, switches, firewalls, and other network infrastructure | Cisco routers, Juniper switches, Palo Alto firewalls | Discovered through [network device discovery](network-devices.md) scans using any network protocol |
...
| **Uncategorized devices** | Devices discovered in your environment that couldn't be classified into a specific category | Unknown devices that responded to discovery probes but lack enough identifying information | Default when discovery can't determine the device type |
| **Partially identified devices** | Devices that Defender for Endpoint detected on the network but couldn't fully identify. These devices have incomplete attributes such as missing hostname, OS, or device category. | Devices that responded minimally to discovery probes, or were only briefly observed on the network | Default when discovery detects a device but can't resolve its full identity |
...
| **Partially identified devices** | Devices detected on the network but not fully identified. | All devices |

Above the counts, the inventory might display special cards:
...
- **Devices not protected**: Shows the number of devices that don't have the Defender for Endpoint agent installed, with a prompt to onboard them.
...
| **Vendor** | No | The manufacturer of the device hardware, as reported by the device or discovered by Defender for Endpoint. Primarily useful for network and IoT/OT devices. | Cisco, HP, Dell |
| **Model** | No | The hardware model of the device, as reported by the device or discovered through probing. Primarily useful for network and IoT/OT devices. | `Catalyst 9300`, `ProLiant DL380` |
...
| **Discovery sources** | Yes | How the device was found and added to the inventory. Tells you the method or integration that detected the device. See [Discovery sources values](#discovery-sources-values). | Defender for Cloud, Defender for Identity, Defender for IoT Network sensor, Microsoft Defender for IoT, MDE, MDATP |
...
| **Last device update** | Yes | The timestamp of the latest LastSeen value reflected in device inventory. This column updates on the inventory refresh cadence (typically up to daily), so it might lag behind fresher last seen times shown in Advanced hunting or on the device page. For discovered devices, this updates when the device is re-observed. | `2/24/2026 3:45 PM` |
...
| **Zone** | No | The Defender for IoT network zone that the device belongs to. Zones represent logical segments within a site. Requires a Defender for IoT license. | `Corporate`, `Production`, `DMZ` |
| **Network sensor** | No | The Defender for IoT network sensor that detected the device. Indicates which sensor is monitoring traffic for this device. Requires a Defender for IoT license. | `Sensor-HQ-01`, `Factory-Sensor-02` |
| **Purdue level** | No | The Purdue model level assigned to the device, indicating its position in the industrial network architecture. Relevant for OT environments. Requires a Defender for IoT license. | Level 0 (Process), Level 1 (Control), Level 2 (Supervisory), Level 3 (Operations) |
...
> [!NOTE]
> Filter order in the portal varies by tab. This reference lists filters in an order that approximates the portal layout on the **All devices** tab.

| Filter | Available on tabs | Description | Values | When to use |
|--------|-------------------|-------------|--------|-------------|
| **Device category** | All devices, IoT/OT, Partially identified | The broad classification of the device — equivalent to the tab the device appears on. | BMS, Computers and Mobile, IoT, Medical, Network Device, OT, Unknown | Use on the **All devices** tab to narrow to a specific device type without switching tabs. Useful when combining with other filters. |
| **Device type** | All devices, IoT/OT, Partially identified | The specific type of device within a category. For computers and mobile, this is typically Workstation or Server. For IoT, categories like Printer, Camera, and Router are common. | Free text entry or select from discovered values (for example, Audio and Video, Printer, Router) | Find all devices of a particular type. For example, filter for all printers to check their security posture. |
...
| **Risk level** | All tabs | The overall risk of the device based on active alerts and threat signals. See [Risk level values](#risk-level-values). | High, Medium, Low, Informational, No known risks | Prioritize investigation. Filter for **High** risk to find devices with active, high-severity alerts that need immediate attention. |
| **Exposure level** | All tabs | How vulnerable the device is based on the number and severity of unresolved security recommendations. See [Exposure level values](#exposure-level-values). | High, Medium, Low, No data available | Prioritize remediation. Filter for **High** exposure to find devices that need immediate patching or configuration changes. |
...
| **OS platform** | All devices, Computers & mobile, Partially identified | The operating system family. Use this for broad OS-level filtering; use **OS distribution** for more specific results. | Windows, macOS, Linux, iOS, Android | Scope your view to a specific OS family. For example, find all Linux servers for a security review. |
...
| **Discovery sources** | All tabs | The mechanism that detected the device and added it to the inventory. This field directly answers the question "how did this device get here?" | Various sources including Defender for Cloud, Defender for Identity, Defender for IoT Network sensor, Microsoft Defender for IoT, MDE, MDATP, Network scan | Understand how devices were found. Filter for non-MDE sources to see devices discovered through integrations, network sensors, or scanning. |
| **First seen** | All devices, Computers & mobile, IoT/OT, Uncategorized | When the device first appeared in your inventory. The actual date the sensor or discovery mechanism originally detected the device. | Last 7 days, Over 7 days ago | Find newly discovered devices that might need investigation or onboarding. Use **Last 7 days** to see recent additions. |
| **Tags** | All devices, Computers & mobile, Network devices, IoT/OT, Uncategorized | Custom labels applied to devices for grouping, filtering, and targeting. Tags can be set manually on individual devices, applied through dynamic tagging rules, or added automatically by the system (for example, `internet-facing`). | Free text entry matching tag names | Filter for a specific business context. For example, find all devices tagged `Finance` or `HQ-Building-A`. See [Create and manage device tags](machine-tags.md). |
...
| **Transient device** | All devices, Computers & mobile, Network devices, IoT/OT, Uncategorized | Whether the device appears intermittently on the network. Transient devices connect briefly and disappear — common with personal devices, guest devices, or devices used temporarily. These devices can add noise to your inventory. | Yes, No | Reduce noise in your inventory. Filter for **No** to focus on persistent devices. Review transient devices periodically. See [Manage device scope and relevance](manage-device-scope-relevance.md). |
| **Partially identified devices** | All devices | Whether the device is only partially identified, meaning Defender for Endpoint detected the device but couldn't fully resolve all its attributes. Partially identified devices might have incomplete information such as missing hostname or OS details. | Yes, No | Filter for **No** to exclude partially identified devices and reduce noise. Filter for **Yes** to review devices that need further investigation or manual classification. |
| **Group** | All devices, Computers & mobile | The device group the device belongs to, as configured in [device groups](machine-groups.md). Device groups control role-based access (RBAC) — security analysts only see devices in their assigned groups. | Free text entry matching configured group names | Scope your view to devices in a specific group. Useful when you manage a subset of devices (for example, a regional group or business unit). |
| **Device value** | All devices, Computers & mobile, Network devices, IoT/OT, Uncategorized | The priority value you assigned to the device. **High**-value devices are weighted more heavily in exposure score calculations. | High, Low | Review which devices you marked as high priority, or find low-value devices that might not need the same level of attention. |
| **Exclusion state** | All devices, Computers & mobile, Network devices, IoT/OT, Uncategorized | Whether the device is excluded from [Microsoft Defender Vulnerability Management](/defender-vulnerability-management/defender-vulnerability-management) assessments. Excluded devices don't affect your exposure score or generate security recommendations. | Not excluded, Excluded | Review your exclusion list periodically. Find excluded devices to determine if any should be brought back into vulnerability management scope. See [Manage device scope and relevance](manage-device-scope-relevance.md). |
| **Firmware version** | All devices, IoT/OT | The firmware version running on the device. Primarily relevant for network and IoT/OT devices. | Free text entry or select from discovered values | Find devices running a specific firmware version, for example to check for a known firmware vulnerability. |
...
| **Purdue level** | All devices, IoT/OT | The Purdue model level assigned to the device, indicating its position in the industrial network architecture. Relevant for OT environments. Requires a Defender for IoT license. | Free text entry | Find devices at a specific Purdue level for security assessments of industrial network segments. |
| **Site** | All devices, IoT/OT | The Defender for IoT site that the device belongs to. Sites represent physical locations or network segments defined in your Defender for IoT configuration. This filter is only visible when a Defender for IoT license is active. | Free text entry matching site names | Scope your view to a specific physical site or location. Useful for facility-specific security reviews. |
| **Zone** | All devices, IoT/OT | The Defender for IoT network zone that the device belongs to. Zones represent logical segments within a site. Requires a Defender for IoT license. | Free text entry matching zone names | Filter devices by network zone for targeted security reviews within a site. |
| **Network sensor** | All devices, IoT/OT | The Defender for IoT network sensor that detected the device. Indicates which sensor is monitoring traffic for this device. Requires a Defender for IoT license. | Free text entry matching sensor names | Identify which devices are monitored by a specific sensor. Useful for troubleshooting sensor coverage gaps. |
...
| **Windows version** | Computers & mobile, Partially identified | The specific Windows version running on the device. Only relevant on the **Computers & mobile** tab. | Various Windows versions (for example, 22H2, 21H2, 1809) | Find devices running an outdated or end-of-support Windows version that needs to be upgraded. |
...
| Value | What it means |
|-------|--------------|
| **Defender for Cloud** | The device was surfaced through Microsoft Defender for Cloud integration. This can include cloud resource and workload context synchronized into device inventory. |
| **Defender for Identity** | The device was surfaced through Microsoft Defender for Identity signals and integration data. |
| **Defender for IoT Network sensor** | The device was detected by a Defender for IoT network sensor observing network traffic. |
| **Microsoft Defender for IoT** | The device was discovered by the broader Defender for IoT integration. Typically an IoT or OT device identified through specialized device fingerprinting. Sensor-specific detections can appear as **Defender for IoT Network sensor**. |
...
> [!NOTE]
> A device can have multiple discovery sources if it was found by more than one mechanism. For example, a device might show both **MDE** (discovered by the sensor) and **Defender for Cloud** (surfaced through cloud integration), or both **Defender for IoT Network sensor** and **Microsoft Defender for IoT**.
