...
| **Sensor health state** | Yes | The current health status of the Defender for Endpoint sensor on onboarded devices. Empty for discovered (non-onboarded) devices. See [Sensor health state values](#sensor-health-state-values). | Active, Inactive, Misconfigured |
...
The **Exposure level** reflects the device's vulnerability posture based on pending security recommendations from [Microsoft Defender Vulnerability Management](../defender-vulnerability-management/defender-vulnerability-management.md).
