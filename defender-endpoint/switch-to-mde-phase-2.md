...
1. If you're using either [Microsoft Intune](/intune/intune-service/fundamentals/what-is-intune) or [Microsoft Configuration Manager](/intune/configmgr/core/understand/introduction) to onboard devices and configure device policies, set up integration with Defender for Endpoint by following these steps: <br/>
    ...
    1. If you're planning to use Configuration Manager, set the toggle for **Manage Security settings using Configuration Manager** to **On**. (If you need help with this step, see [Coexistence with Microsoft Endpoint Configuration Manager](/intune/intune-service/protect/mde-security-integration#co-existence-with-microsoft-endpoint-configuration-manager).)
...
1. Configure your [next-generation protection capabilities](next-generation-protection.md).
   ...
   |Control Panel in Windows|Follow the guidance here: [Turn on Microsoft Defender Antivirus](/intune/intune-service/user-help/turn-on-defender-windows). (You might see *Windows Defender Antivirus* instead of *Microsoft Defender Antivirus* in some versions of Windows.)|
