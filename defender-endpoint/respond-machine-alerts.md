...
- You can use the device isolation capability on all supported Microsoft Defender for Endpoint on Linux listed in [System requirements](/defender-endpoint/mde-linux-prerequisites). Ensure that the following prerequisites are enabled:
   - `iptables`
   - `ip6tables`
