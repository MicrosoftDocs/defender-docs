...
> [!NOTE]
> When any advanced troubleshooting parameter is used, the analyzer also calls into [MpCmdRun.exe](command-line-arguments-microsoft-defender-antivirus.md) to collect Microsoft Defender Antivirus related support logs.
> You can use `-g` flag to validate URLs for a specific datacenter region even without being onboarded to that region.
