...
1. Enter details and create a new group.
...
1. Add your test user or device.

1. From the **Groups > All groups** pane, open your new group.

1. Select  **Members > Add members**.
1. Find your test user or device and select it.
...
1. Your testing group now has a member to test.
...
1. Navigate to **Endpoint security > Endpoint detection and response**. Select on **Create Policy**.
...
1. Under **Platform, select Windows 10, Windows 11, and Windows Server, Profile - Endpoint detection
    and response > Create**.
...
1. Add scope tags if necessary, then select  **Next**.
...
1. Add test group by clicking on **Select groups to include** and choose your group, then select  **Next**.
...
1. Review and accept, then select  **Create**.
...
1. You can view your completed policy.
...
1. Select **Platform - Windows 10 and Later - Windows and Profile - Microsoft Defender Antivirus > Create**.

1. Enter name and description, then select  **Next**.
...
1. In the **Configuration settings page**: Set the configurations you require for Microsoft Defender Antivirus (Cloud Protection, Exclusions, Real-Time Protection, and Remediation).
...
1. Select groups to include, assign to your test group, then select  **Next**.
...
1. Review and create, then select  **Create**.
...
1. You see the configuration policy you created.
...
1. Select **Platform - Windows 10 and Later - Profile - Attack surface reduction
    rules > Create**.
...
1. In the **Configuration settings page**: Set the configurations you require for Attack surface reduction rules, then select  **Next**.
...
1. Add Scope Tags as required, then select  **Next**.
...
1. Select groups to include and assign to test group, then select  **Next**.
...
1. Review the details, then select  **Create**.
...
1. Select **Windows 10 and Later - Web protection > Create**.
...
1. In the **Configuration settings page**: Set the configurations you require for
...
1. Add **Scope Tags as required > Next**.
...
1. Select **Assign to test group > Next**.
...
1. Select  the **Configuration Policy** to view the policy status.
...
1. Select  **Device Status** to see the status.
...
1. Select  **User Status** to see the status.
...
1. Select  **Per-setting status** to see the status.
...
1. After the configuration is applied, the Defender for Endpoint Protection service should be started.

    > [!div class="mx-imgBorder"]
    > [![Image of Services panel2.](media/a621b699899f1b41db211170074ea59e.png)](media/a621b699899f1b41db211170074ea59e.png#lightbox)

1. After the services are running on the device, the device appears in Microsoft Defender portal.
...
1. After the policy is applied, you shouldn't be able to manually manage
    the settings.
...
1. You should see the following lines with no content:
...
1. After applying the policy on a test device, open a PowerShell Windows and type `Get-MpPreference`.

1. You should see the following lines with content, as shown in the following image:
...
1. This should respond with a 0 as shown in the following image:
...
1. After applying the policy, open a PowerShell Windows and type
    `(Get-MpPreference).EnableNetworkProtection`.

1. You should see a response with a 1 as shown in the following image:
