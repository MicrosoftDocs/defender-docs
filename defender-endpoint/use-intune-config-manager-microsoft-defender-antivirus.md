...
4. Click on **Create Policy**, select **Windows** as the platform and **Microsoft Defender Antivirus** for the profile type then select on **Create**.
...
### Allow Archive Scanning
  ...
  - **Not allowed** - Archive files aren't scanned,but they are always scanned during directed scans.
  - **Allowed** - Enable scans of archive files. This is the recommended configuration.

  Changes to this setting will not be applied if tamper protection is enabled.
...
### Allow behavior monitoring
  ...
  - **Not configured** - The setting is reverts to the system default (real-time behavior monitoring is enabled).
...
Information about the detected items on your computer is automatically collected and sent to Microsoft. The following information is collected about any detected malicious software, spyware, and potentially unwanted software: where the software came from, the actions that you apply or that are applied automatically and whether the actions were successful, the location of the software, file names, how the software operates and how it has impacted your computer.

  - **Not configured** - The setting is reverts to the system default (cloud-delivered protection is turned off).
...
### Allow email scanning 
  ...
  - **Not configured** - The setting is reverts to the system default (turn off email scanning).
...
### Allow Full Scan On Mapped Network Drives
  ...
  - **Not configured** - The setting is reverts to the system default (mapped network drives won't be scanned).
...
### Avg CPU Load Factor
  ...
  - **Not configured** - The setting reverts to the system default, where CPU utilization does not exceed the default value of 50%.
  - **[0-100]** - CPU utilization will not exceed the specified percentage. A value of 0 means there is no throttling of CPU utilization.
...
### Days To Retain Cleaned Malware 
  ...
  - **Not configured** - The setting reverts to the system default. Items is kept in the quarantine folder indefinitely and won't be automatically removed.
  - **[0-90]** - Items is removed from the Quarantine folder after the number of days specified.
...
### Disable Catchup Full Scan
  ...
  - **Disabled** - Catch-up scans for scheduled full scans is turned on. If a computer is offline for two consecutive scheduled scans, a catch-up scan is started the next time someone sign-ins the computer. If there's no scheduled scan configured, there is no catch-up scan run.
...
### Disable Catchup Quick Scan
  ...
  - **Disabled** - Catch-up scans for scheduled quick scans is turned on. If a computer is offline for two consecutive scheduled scans, a catch-up scan is started the next time someone sign-ins the computer. If there's no scheduled scan configured, there is no catch-up scan run.
...
### Enable Network Protection
  ...
  Enable or disable Microsoft Defender Exploit Guard network protection to prevent employees from usng applications to access dangerous domains that might host phishing scams, exploit-hosting sites, and other malicious content on the internet.
  
  - **Not configured** - The setting reverts to the system defaultt, meaning users and applications won't be blocked from connecting to dangerous domains.
...
> [!NOTE]
> When tamper protection is turned on the exclusions can't be modified and new exclusions can't be added but they need to [meet some conditions](./manage-tamper-protection-intune.md) for tamper protection to work.

> [!IMPORTANT]
> Defining exclusions lowers the protection offered by Microsoft Defender Antivirus. You should always evaluate the risks that are associated with implementing exclusions and you should only create exclusions on the devices where they're needed. If an exclusion isn't required on all devices, then use the dedicated policy type **Microsoft Defender Antivirus exclusions** and assign the policy just to the group of devices/users that need it.
...
### PUA Protection
  ...
  - **Not configured** - The setting reverts to the system default (potentially unwanted software won't be blocked).
  - **Disabled** - PUA protection is off, potentially unwanted software won't be blocked.
  - **Block** - PUA protection is on, potentially unwanted software is blocked. This is the recommended configuration.
  - **Audit mode** - Potentially unwanted software won't be blocked, however if this feature would have blocked access if it were set to Block, then a record of the event is in the event logs.

### Real Time Scan Direction  
   CSP: [RealTimeScanDirection](/windows/client-management/mdm/policy-csp-defender#realtimescandirection)

  This policy setting enables you to configure monitoring for incoming and outgoing files, without having to turn off monitoring entirely. It's recommended for use on servers where there's numerous incoming and outgoing file activity but for performance reasons need to have scanning disabled for a particular scan direction. The appropriate configuration should be evaluated based on the server role.

  This configuration is only honored for NTFS volumes. For any other file system type, full monitoring of file and program activity is present on those volumes.
  
  - **Not configured** - The setting reverts to the system default (monitoring for incoming and outgoing files are enabled).
...
### Scan Parameter
  ...
  This policy setting enables you to specify the scan type to use during a scheduled scan. This setting interacts with the settings **Schedule Scan Day** and **Schedule Scan Time**.

  - **Not configured** - The setting reverts to the system default (the default scan type is used).
...
### Schedule Quick Scan Time
  ...
  This policy setting enables you to specify the time of day at which to perform a daily quick scan. The time value is represented as the number of minutes past midnights. This setting doesn't interact with the settings **Scan Parameter**, **Schedule Scan Day** and **Schedule Scan Time**.

  - **Not configured** - The setting reverts to the system default (the daily quick scan controlled by this config won't run).
...
### Schedule Scan Day
  ...
  This policy setting enables you to specify the day of the week on which to perform a scheduled scan. The scan can also be configured to run every day or to never run at all. This setting interacts with the settings **Scan Parameter** and **Schedule Scan Time**.

  - **Not configured** - The setting reverts to the system default (a scheduled scan runs at a default frequency).
...
### Schedule Scan Time
  ...
  This policy setting enables you to specify the time of day at which to perform a scheduled scan. The time value is represented as the number of minutes past midnights (the default is 120 which corresponds to 2:00 AM). This setting interacts with the settings **Scan Parameter** and **Schedule Scan Day**.
...
### Signature Update Fallback Order
  ...
  This policy setting enables you to define the order in which different security intelligence update sources should be contacted. The value of this setting should be entered as a pipe-separated string enumerating the security intelligence update sources in order. Possible values are: "InternalDefinitionUpdateServer," "MicrosoftUpdateServer," "MMPC," and "FileShares".

  - **Not configured** - The setting reverts to the system default (security intelligence update sources is contacted in a default order).
...
### Signature Update File Shares Sources
  ...
  This policy setting enables you to configure UNC file share sources for downloading security intelligence updates. Sources are contacted in the order specified. The value of this setting should be entered as a pipe-separated string enumerating the security intelligence update sources.

  - **Not configured** - The setting reverts to the system default (the list remains empty by default and no sources are contacted).
...
### Signature Update Interval
  ...
  This policy setting enables you to specify an interval at which to check for security intelligence updates. The time value is represented as the number of hours between update checks (the default is 8h).

  - **Not configured** - The setting reverts to the system default (checks for security intelligence updates occur at the default interval).
...
### Submit Samples Consent
  ...
  - **Not configured** - The setting reverts to the system default (send safe samples automatically).
...
### Disable Local Admin Merge
  ...
  When this value is set to no, it enables a local admin the ability to [specify some settings](./configure-local-policy-overrides-microsoft-defender-antivirus.md) on their devices by using the Windows Security app, local Group Policy settings, or PowerShell cmdlets (where appropriate).
...
### Allow On Access Protection
  ...
  - **Not configured** - The setting reverts to the system default (monitoring for file and program activity is enabled).
...
### Threat Severity Default Action#
  ...
  This policy setting enables you to customize which automatic remediation action is taken for each threat alert level. The list of valid remediation actions is below.

  - **Not configured** - The setting reverts to the system default (apply action based on the update definition).
...
### Allow Network Protection Down Level
  ...
  This setting controls whether Network Protection is allowed to be configured into block or audit mode on Windows downlevel of RS3. If false, the value of EnableNetworkProtection is ignored.

  - **Not configured** - The setting reverts to the system default (network protection is disabled downlevel).
...
### Allow Datagram Processing On Win Server
  ...
  This setting controls whether Network Protection is allowed to enable datagram processing on Windows Server. If false, the value of DisableDatagramProcessing is ignored and default to disabling Datagram inspection.

  - **Not configured** - The setting reverts to the system default (datagram processing on Windows Server is disabled).
...
### Disable Dns Over Tcp Parsing 
  ...
  - **Not configured** - The setting reverts to the system default (DNS over TCP parsing is enabled).
...
### Disable Http Parsing
  ...
  - **Not configured** - The setting reverts to the system default (HTTP parsing is enabled).
...
### Disable Tls Parsing 
  ...
  - **Not configured** - The setting reverts to the system default (TLS parsing is enabled).
...
### Engine Updates Channel
  ...
  - **Not configured** - The setting reverts to the system default (the device stays up to date automatically during the gradual release cycle). Suitable for most devices.
...
### Disable Core Service ECS Integration
  ...
  - **Not configured** - The setting reverts to the system default (the Defender core service uses ECS).
...
### Disable Core Service Telemetry
  ...
  - **Not configured** - The setting reverts to the system default (the Defender core service uses the OneDsCollector framework).
...
> [!TIP]
> **Performance tip** Due to various factors (examples listed) Microsoft Defender Antivirus, like other antivirus software, can cause performance issues on endpoint devices. In some cases, you might need to tune the performance of Microsoft Defender Antivirus to alleviate those performance issues. Microsoft's **Performance analyzer** is a PowerShell command-line tool that helps determine which files, file paths, processes, and file extensions might be causing performance issues; some examples are:
>
