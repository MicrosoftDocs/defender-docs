...
- **Allow Archive Scanning**  
  ...
  - **Not configured** - The setting returns to the client default, which is to scan archived files, however the user might disable setting.
  - **Not allowed** - Archive files aren't scanned. However, archives are always scanned during directed scans.
...
- **Allow email scanning**  
  ...
  This policy setting allows you to configure e-mail scanning. When e-mail scanning is enabled, the engine parses the mailbox and mail files, according to their specific format, in order to analyze the mail bodies and attachments. Several e-mail formats are currently supported, for example: pst (Outlook), dbx, mbx, mime (Outlook Express), binhex (Mac). Email scanning isn't supported on modern email clients.
...
- **Allow Full Scan On Mapped Network Drives**  
  ...
  - **Not configured** - The setting is restored to the system default (mapped network drives won't be scanned).
...
- **Allow Full Scan Removable Drive Scanning**  
  ...
  - **Not configured** - The setting is restored to the system default (removable drives won't be scanned during a full scan but they might still be scanned during quick scan and custom scan).
...
- **Allow scanning of all downloaded files and attachments**  
  ...
  - **Not configured** - The setting is restored to the system default (scanning for all downloaded files and attachments are enabled).
...
- **Allow Script Scanning**  
  ...
  - **Allowed** - The setting is enabled, scripts are scanned(it enables the [Anti-malware Scan Interface](amsi-on-mdav.md)). This is the recommended configuration.
...
- **Avg CPU Load Factor**  
  ...
  - **Not configured** - The setting is restored to the system default (CPU utilization doesn't exceed the default value which is 50).
  - **[0-100]** - CPU utilization doesn't exceed the percentage specified. A value of 0 indicates that there should be no throttling of CPU utilization.
...
- **Check For Signatures Before Running Scan**  
  ...
  - **Not configured** - The setting is restored to the system default (the scan starts using the existing security intelligence).
  - **Disabled** - The scan starts using the existing security intelligence.
  - **Enabled** - A check for new security intelligence occurs before running a scan. This is the recommended configuration.
 
- **Cloud Block Level**  
   CSP: [CloudBlockLevel](/windows/client-management/mdm/policy-csp-defender#cloudblocklevel)

  This policy setting determines how aggressive Microsoft Defender Antivirus is in blocking and scanning suspicious files. This feature requires **Allow cloud protection** to be enabled.
...
- **Cloud Extended Timeout**  
  ...
  This feature allows Microsoft Defender Antivirus to block a suspicious file for up to 60 seconds, and scan it in the cloud to make sure it's safe. The default cloud check time-out is 10 seconds. To enable the extended cloud check feature, specify the extended time in seconds, up to an additional 50 seconds.
  
  - **Not configured** - The setting is restored to the system default (cloud time out is 10 seconds).
...
- **Disable Catchup Full Scan**  
  ...
  - **Disabled** - Catch-up scans for scheduled full scans will be turned on. If a computer is offline for two consecutive scheduled scans, a catch-up scan is started the next time someone sign-ins the computer. If there's no scheduled scan configured, there will be no catch-up scan run.
...
- **Disable Catchup Quick Scan**  
  ...
  - **Disabled** - Catch-up scans for scheduled quick scans will be turned on. If a computer is offline for two consecutive scheduled scans, a catch-up scan is started the next time someone sign-ins the computer. If there's no scheduled scan configured, there will be no catch-up scan run.
...
- **Enable Low CPU Priority**  
  ...
  - **Disabled** - No changes are made to CPU priority for scheduled scans.
...
- **Enable Network Protection**  
  ...
  Enable or disable Microsoft Defender Exploit Guard network protection to prevent employees from using any application to access dangerous domains that might host phishing scams, exploit-hosting sites, and other malicious content on the Internet.
  ...
  - **Enabled(audit mode)** - Users and applications can connect to dangerous domains, however if this feature would have blocked access if it were set to Block, then a record of the event will be in the event logs.
...
- **PUA Protection**  
  ...
  - **Audit mode** - Potentially unwanted software won't be blocked, however if this feature would have blocked access if it were set to Block, then a record of the event will be in the event logs.
...
- **Real Time Scan Direction**  
  ...
  - **Not configured** - The setting is restored to the system default (monitoring for incoming and outgoing files are enabled).
...
- **Schedule Quick Scan Time**  
  ...
  - **Not configured** - The setting is restored to the system default (the daily quick scan controlled by this config won't run).
...
- **Schedule Scan Day**  
  ...
  - **Not configured** - The setting is restored to the system default (a scheduled scan runs at a default frequency).
...
- **Schedule Scan Time**  
  ...
  This policy setting allows you to specify the time of day at which to perform a scheduled scan. The time value is represented as the number of minutes past midnights (the default is 120 which corresponds to 2:00 AM). This setting interacts with the settings **Scan Parameter** and **Schedule Scan Day**.

  - **Not configured** - The setting is restored to the system default (a scheduled scan runs at a default time).
...
- **Signature Update File Shares Sources**  
  ...
  - **Not configured** - The setting is restored to the system default (the list remains empty by default and no sources are contacted).
...
- **Signature Update Interval**  
  ...
  - **Not configured** - The setting is restored to the system default (checks for security intelligence updates occur at the default interval).
...
- **Engine Updates Channel**  
  ...
  - **Not configured** - The setting is restored to the system default (the device stays up to date automatically during the gradual release cycle). Suitable for most devices.
...
- **Randomize Schedule Task Times**  
  ...
  - **Not configured** - The setting is restored to the system default (scheduled tasks are randomized).
...
- **Disable Core Service ECS Integration**  
  ...
  - **Not configured** - The setting is restored to the system default (the Defender core service uses ECS).
...
- **Disable Core Service Telemetry**  
  ...
  - **Not configured** - The setting is restored to the system default (the Defender core service uses the OneDsCollector framework).
