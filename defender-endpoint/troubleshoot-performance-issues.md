...
|5. **Misspelled exclusions**. <br/>|Use `MpCmdRun.exe -CheckExclusion -Path <PathAndFile or Path>` to validate path-based exclusions. For more information, see [Verify whether a file or folder is excluded by using MpCmdRun](microsoft-defender-antivirus-exclusions-configure.md#verify-whether-a-file-or-folder-is-excluded-by-using-mpcmdrun).|
