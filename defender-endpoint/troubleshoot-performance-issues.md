...
|5. **Misspelled exclusions**. <br/>|Use `MpCmdRun.exe -CheckExclusion -Path <path>` to validate path-based exclusions. For more information, see [Verify whether a specified path is excluded using MpCmdRun](configure-extension-file-exclusions-microsoft-defender-antivirus.md#verify-whether-a-specified-path-is-excluded-using-mpcmdrun).|
