...
> 
> Starting in February 2026, Microsoft Defender Antivirus on Windows is changing how antivirus settings (like exclusions) are stored when Microsoft Defender for Endpoint configuration management is enabled on a tenant. Starting with the 4.18.25110.6 release, organizations using Microsoft Defender for Endpoint configuration management will no longer be able to read exclusion values directly from the local device registry. Instead, setting configuration must be retrieved using supported Microsoft Defender PowerShell cmdlets. Organizations using MDE configuration management must use supported Defender PowerShell cmdlets (such as Get-MpPreference).
