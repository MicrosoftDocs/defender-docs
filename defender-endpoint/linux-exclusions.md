...
|Process|A specific process (specified either by the full path or file name) and all files opened by it.<br/>A*ntivirus exclusions can be added using either a full path or file name but for global exclusions please only use full and trusted process launch path. In both the cases, Microsoft recommendation is to use the full path.*|`/bin/cat`<br/>`cat`<br/>`c?t`|
