...
Microsoft Sentinel is a cloud-native security information and event management (SIEM) solution that can ingest, curate, and manage threat intelligence from numerous sources.

[!INCLUDE [unified-soc-preview](includes/unified-soc-preview.md)]

## Introduction to threat intelligence

Cyber threat intelligence (CTI) is information that describes existing or potential threats to systems and users. This intelligence takes many forms, such as written reports that detail a particular threat actor's motivations, infrastructure, and techniques. It can also be specific observations of IP addresses, domains, file hashes, and other artifacts associated with known cyber threats.
...
Most threat intelligence is imported through data connectors or an API. Configure ingestion rules for data connectors to reduce noise and ensure your intelligence feeds are optimized. Here are the solutions available for Microsoft Sentinel.
...
Many organizations use threat intelligence platform (TIP) solutions to aggregate threat indicator feeds from various sources. From the aggregated feed, the data is curated to apply to security solutions such as network devices, EDR/XDR solutions, or SIEMs such as Microsoft Sentinel. By using the **upload API**, you can use these solutions to import threat intelligence STIX objects into Microsoft Sentinel. 
...
>[!NOTE]
> You still access threat intelligence in the Azure portal from **Microsoft Sentinel** > **Threat management** > **Threat intelligence**.

Two of the most common threat intelligence tasks are creating new threat intelligence related to security investigations and adding tags. The management interface streamlines the manual process of curating individual threat intel by using a few key features.

- Configure ingestion rules to optimize threat intel from data connector sources.
- Define relationships as you create new STIX objects.
- Curate existing TI by using the relationship builder.
- Copy common metadata from a new or existing TI object by using the duplicate feature.
- Add free-form tags to objects by using multi-select.
...
You can optimize threat intelligence from data connectors by filtering and enhancing objects before they're delivered to your workspace. Ingestion rules apply to data connectors only and don't affect threat intelligence added through the upload API or created manually. Ingestion rules update attributes or filter objects out altogether. The following table lists some use cases:

| Ingestion rule use case | Description |
|---|---|
| Reduce noise | Filter out old threat intelligence not updated for six months that also has low confidence. |
...
Keep the following tips in mind when using ingestion rules:
...
You can enhance threat detection and response by establishing connections between objects by using the relationship builder. The following table lists some of its use cases:
...
Configure which TI objects you can share with appropriate audiences by designating a sensitivity level called Traffic Light Protocol (TLP). 
...
View your threat intelligence from the management interface or by using queries: 

- From the management interface, use advanced search to sort and filter your threat intelligence objects without writing a Log Analytics query.
