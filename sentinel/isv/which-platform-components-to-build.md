---
title: Decide which platform solution components to build
description: Select the right Microsoft Sentinel components for your ISV solution. Learn when to use Security Copilot agents, MCP tools, graphs, and notebook jobs.
author: EdB-MSFT
ms.author: edbaynash
...
#customer intent: As an ISV partner, I want to choose the right Microsoft Sentinel platform components so that I can design and publish a solution that fits my customer scenario.
---

# Decide which platform solution components to build

The Microsoft Sentinel platform is a cloud-native foundation for security data, analytics, and AI that Independent Software Vendor (ISV) partners build on to deliver security solutions to customers. It unifies a scalable security data lake, the investigation and incident surface, and automation and AI capabilities into a single extensible platform.
...
Use this guidance if you've decided to integrate with Microsoft Sentinel platform and need to choose which components to combine for your scenario.


## Microsoft Sentinel platform components

Microsoft Sentinel provides a normalized security data layer, an incident and entity investigation surface, and automation and AI capabilities. ISV partners can use these components separately or combine them.
...
Start from customer benefit instead of technology. Choose components by answering the following questions:
...
| Customer requirement | Solution option | Best-fit component |
|---|---|---|
...
|---|---|---|
| Identity protection or Identity Threat Detection and Response (ITDR)  | Determine whether a risky sign-in is part of an attack path by correlating identity risk, MFA logs, sign-in success and failure, geographic drift, and post-auth endpoint execution. | Identity-centric, last-24-hour correlation where analysts paste a UPN and need a verdict. |
...
| Secure access, Security Service Edge (SSE), or Zero Trust Network Access (ZTNA) | Determine whether user activity is legitimate remote access or suspicious connectivity toward private apps. | Behavior-drift reasoning using access telemetry, MFA outcomes, and destination categories with narrative output. |
...
| Asset exposure, Cloud Security Posture Management (CSPM), or vulnerability | Determine exploitation likelihood and blast radius for a workload. | Combines exposure facts with alert activity and returns a resource-specific risk narrative. |
...
For a detailed guide, see [Develop a Security Copilot agent platform solution](build-agent-security-copilot.md).
...
|---|---|---|
| Identity protection or ITDR | `get_user_signin_summary(UserPrincipalName, workspaceId)` | An ISV product console agent that enriches an alert without reimplementing KQL. |
...
- 
For a detailed guide, see [Develop custom Graph platform solutions](develop-custom-graph-platform-solutions.md)

## Jupyter notebook job concepts

### When to choose a notebook job

Use a notebook job for deterministic, scheduled, compute-heavy workloads that run in the Sentinel data lake and produce outputs consumed by other components.
...
- A notebook that reads lake data with pySpark, runs transformations or models, and writes results to a custom table or graph instance.
...
For a detailed guide, see [Develop notebook platform solutions](develop-notebook-platform-solutions.md)

## Solution composition patterns

High-impact ISV solutions combine components. The following patterns are common in ISV engagements.

### Connector plus analytics rules plus Security Copilot agent
...
This pattern is best for any ISV that already has a Sentinel solution in Content Hub and wants to extend it with an agent.

### Connector plus notebook job plus custom table plus Security Copilot agent
...
This pattern is best for UEBA, anomaly scoring, or scenarios where per-run KQL is too expensive or nondeterministic.

### Connector plus notebook job plus custom graph plus Security Copilot agent
...
This pattern is best for ISVs whose value proposition depends on relationship reasoning, such as identity blast radius, asset reachability, and application permission graphs.

### Custom MCP tool collection plus ISV product agent
...
This pattern is best for ISVs that already ship their own agent and want deterministic, governed access to Sentinel data in each customer tenant.

### Security Copilot agent that calls a custom MCP tool collection
...
Use the following checklist to narrow components for your scenario.

| Question | If yes, consider |
|---|---|
| Has the data been ingested into Sentinel? | A Sentinel data connector first. Other components depend on this. |
| Is the primary consumer a SOC analyst who pastes an entity into chat? | Security Copilot agent. |
...
| Is per-run KQL too expensive, too slow, or nondeterministic inside an agent skill? | Notebook job that precomputes a custom table consumed at query time. |
...
After you choose components, continue with capability-specific guidance:
...
If you need help selecting components for your scenario, use the Sentinel Advisory Service intake form.
