...
Because this feature is built into the latest version of the Microsoft Sentinel API, the alert-to-incident capability is also available through the Logic Apps connector for Microsoft Sentinel. So you can use playbooks to automatically add an alert to an incident if certain conditions are met.
...
- The alert you want to add belongs to another incident, and it's the only alert in the other incident.
    ...
    This situation differs from the scenario where the other incident contains multiple alerts, since if the alert is alone in the other incident, tracking it in this incident could make the other incident irrelevant. So in this case, you'll see this dialog:
