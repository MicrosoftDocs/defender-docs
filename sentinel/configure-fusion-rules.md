...
1. Fusion for emerging threats uses alerts generated by any [scheduled analytics rules](scheduled-rules-overview.md) that contain kill-chain (tactics) and entity mapping information. To ensure that Fusion can use alerts from a scheduled analytics rule to detect emerging threats, verify the rule's entity mapping and tactics configuration:
