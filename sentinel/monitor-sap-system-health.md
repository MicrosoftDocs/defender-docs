...
The Microsoft Sentinel for SAP solution includes an alert rule template. This template helps you track the health of your SAP agent's data collection.

The rule needs at least seven days of history to detect usage patterns. We recommend setting the alert rule **Look back** parameter to 14 days. This setting helps detect weekly activity profiles.

Once active, the rule checks recent telemetry and log volume against past trends. It then alerts on issues and assigns severity based on the scope of the problem.
...
- Checks signals sent from the agent.
- Checks telemetry data.
- Alerts on log gaps and other connectivity issues, if any exist.
- Learns from log history, and works better over time.
