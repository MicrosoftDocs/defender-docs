...
- **Preview status and updates:** The UEBA behaviors layer and the Playbook Generator were introduced as new capabilities and are subject to ongoing enhancements. During their Preview phase, they might receive frequent updates and improvements. Their capabilities or interfaces might evolve as Microsoft incorporates feedback and advances the technology. Users should monitor the Microsoft Sentinel release notes and documentation for changes, and be prepared to adjust their usage (for example, updating detection rules if a behavior's structure changes, or learning about expanded triggers when they become available for playbook generation).
...
- **Be aware of the risk of overreliance:** Overreliance on AI happens when users accept incorrect or incomplete AI outputs, mainly because mistakes in AI outputs might be hard to detect. For the end-user, overreliance could result in decreased productivity, loss of trust, application abandonment, financial loss, psychological harm, physical harm, among others. In the context of Microsoft Sentinel SIEM, overreliance might lead analysts to treat a neutral behavior observation as a confirmed threat without further investigation, or to assume that the absence of a behavior means no suspicious activity occurred.
...
- [Generate playbooks using AI in Microsoft Sentinel (Preview)](create-playbooks-from-copilot.md): How-to guide for using the AI-powered Playbook Generator, including examples and requirements.
