...
| **Permissions to create an Azure Key Vault or access an existing one** | Use Azure Key Vault to store secrets required to connect to your SAP system. |Required if you plan to store the SAP system credentials in Azure Key Vault. <br><br>Optional if you plan to store them in a configuration file.|
