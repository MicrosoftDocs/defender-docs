...
| **collect-user-master-data** | Determines whether User Master data is ingested or not to the tables `ABAPAuthorizationDetails` and `ABAPUserDetails`. | **true**: Ingested<br>**false**: Not ingested | **true** || **force-audit-log-to-read-from-all-clients** | Determines whether the Audit Log is read from all clients. | **true**: Read from all clients<br>**false**: Not read from all clients | **false** |
