...
Microsoft Sentinel incidents are files that contain an aggregation of all the relevant evidence for specific investigations. Each incident is created (or added to) based on pieces of evidence ([alerts generated by built-in detection rules](detect-threats-built-in.md)) that were either generated by analytics rules or imported from third-party security products that produce their own alerts. Incidents inherit the [entities identified in Microsoft Sentinel](entities.md) contained in the alerts, as well as the alerts' properties, such as severity, status, and MITRE ATT&CK tactics and techniques.
...
Before you investigate incidents, make sure you have the following roles and access:
...
If you're currently viewing the [legacy incident investigation experience](investigate-cases.md) of the incident details page, toggle on the new experience at the top-right of the page to continue with the procedures in this article for the new experience.
...
| **Action**           | **Description**    |
|---------------------------|----------------------------------------------------|
| **Run playbook**          | Run a [Microsoft Sentinel playbook](automate-responses-with-playbooks.md#rplaybook-templates-preview) on this incident to take particular [enrichment, collaboration, or response actions](automate-responses-with-playbooks.md#recommended-use-cases).   |
| **Create automation rule**| Create a [Microsoft Sentinel automation rule](automate-incident-handling-with-automation-rules.md#common-use-cases-and-scenarios) that runs only on incidents like this one (generated by the same analytics rule) in the future.                        |
...
The left-hand incident details panel can also be collapsed into the left margin of the screen by selecting the small, left-pointing double arrow next to the **Owner** drop-down. Even when the left-hand panel is minimized, you can still change the owner, status, and severity.
...
The **Incident timeline** widget shows you the timeline of alerts and [hunting bookmarks](bookmarks.md) in the incident, which can help you reconstruct the timeline of attacker activity.
...
If you already know that a particular entity is a known indicator of compromise, select the three dots on the entity's row and choose **Add to TI** to [add the entity to your threat intelligence](add-entity-to-threat-intelligence.md). (This option is available for [supported entity types for investigation actions](incident-investigation.md#supported-actions).)

If you want to [trigger an automatic response sequence for a particular entity](respond-threats-during-investigation.md), select the three dots and choose **Run playbook (Preview)**. (This option is available for [supported entity types for incident investigation](incident-investigation.md#supported-actions).)
...
- **Timeline** contains a list of the alerts, [hunting bookmarks](bookmarks.md), and [anomalies](soc-ml-anomalies.md) that feature this entity, and also activities the entity has performed, as collected from logs in which the entity appears. All alerts featuring this entity are in this list, *whether or not* the alerts belong to this incident.
...
1. Save the marked results as a bookmark. You have two options to do this:
    ...
    - Select **Add bookmark to the current incident** to create a bookmark and add it to the open incident. Follow the [Microsoft Sentinel hunting bookmark instructions](bookmarks.md) to complete the process. Once completed, the bookmark appears in the incident timeline.
...
If you prefer a visual, graphical representation of alerts, entities, and the connections between them in your investigation, you can use the classic investigation graph to review entities, expand related alerts, view timelines, and explore entity relationships. The downside of the graph is that you end up having to switch contexts a great deal more.
...
:::image type="content" source="media/investigate-incidents/comments-screen.png" alt-text="Screenshot of the incident activity log pane showing existing comments and a rich text editor to add a new comment.":::
