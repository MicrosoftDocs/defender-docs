...
- The **Result** value for these messages on **User reported** tab of the **Submissions** page at <https://security.microsoft.com/reportsubmission?viewid=user> will be [one of the following](submissions-result-definitions.md) after Microsoft analysis. 
...
- **Monitor reported messages in Outlook** is selected: The following configurations are supported:
  ...
  - Use the built-in **Report** button in [supported versions of Outlook](submissions-outlook-report-messages.md#use-the-built-in-report-button-in-outlook) in virtually all Outlook platforms to report email messages.
  ...
  - Use a third-party, non-Microsoft add-in to report email messages.
    - Configure user reported messages from third-party reporting mailbox to Microsoft.
    - Decide whether to customize the feedback email that's sent to users after an admin reviews and marks the message on the **User submissions** tab on the **Submissions** page.
    - Decide whether users can report email messages from quarantine as they release quarantined messages.
...
- **Reported message destinations** section \> **Send the reported messages to**: Select one of the following options:
  ...
  - **Microsoft and my reporting mailbox**: User reported messages go to Microsoft for analysis and to the specified reporting mailbox for an admin or security operations team to analyze.
...
If you're using a third-party reporting button for end users (for example, KnowBe4 Phish Alert Button, Cofense Report Phishing, Hoxhunt report button, or PhishAlarm), you can benefit from the power of Defender incident management, Microsoft analysis, in-product phishing triage, and native automated response capabilities by integrating the reporting button with Microsoft Defender for Office 365. The message formatting requirements for third-party report button are described in the next section.
...
- **Reported message destinations** section \>  **Send reported messages to:** Select one of the following options:
...
In this example, the email address of the reporting mailbox is thirdpartyreporting@wingtiptoys.com in Exchange Online (you can't specify an external email address) and reported message destination is My reporting mailbox only.
...
- The built-in **Report** button are unavailable in all Outlook platforms.
