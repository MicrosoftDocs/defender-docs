...
- URLs misidentified by Defender for Office 365 via the [URL tab in the submission explorer](submissions-admin#report-good-urls-to-microsoft). URL allows entries are enforced at time of click in Teams when [Teams integration in Safe Links policies is turned on](mdo-support-teams-quick-configure.md#step-2-verify-safe-links-integration-for-microsoft-teams).
- Files by using the SHA256 hash value from the [email attachment tab in the submission explorer](submissions-admin#report-good-email-attachments-to-microsoft)
