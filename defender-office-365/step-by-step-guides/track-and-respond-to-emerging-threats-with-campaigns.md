...
<a name="investigating-a-suspicious-email-campaign-using-threat-reports"></a>
...
1. Below the flow diagram, select the **URL clicks** tab to display information regarding the click. On the **URL clicks** tab, you can see the user that clicked on a URL, if the user is tagged as a priority account user, the URL itself, and the time of click.
...
1. If you would like to take an action and move the messages out of the inbox, you can select the message and then select :::image type="icon" source="../media/defender-portal-icon-take-actions.png" border="false"::: **Take action** \> **Move to junk folder**. Moving the message to the Junk Email folder helps ensure the user doesn't continue to interact with the malicious message, which could result in a potential breach.
