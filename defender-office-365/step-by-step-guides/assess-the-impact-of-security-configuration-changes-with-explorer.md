...
Perform the following steps to assess the impact of changing normal confidence phish delivery to quarantine:
...
1. The chart should refresh, and if it now displays no data, that result means no threats were detected in the previously shown mail, which indicates an override isn't needed, as there's no detection to override.
