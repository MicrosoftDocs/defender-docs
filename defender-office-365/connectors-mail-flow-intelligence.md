...
Microsoft 365 tried to connect to the smart host specified in the connector, but the DNS query to find the smart host's IP addresses failed. The possible causes for this error are:

- Issues with your domain's DNS hosting service (the party that maintains the authoritative name servers for your domain).
- Your domain recently expired, so the MX record can't be retrieved.
- Your domain's MX record recently changed, and the DNS servers still hold previously cached DNS information for your domain.

### How do I fix error code 450 4.4.312?

- Work with your DNS hosting service to identify and fix the problem with your domain.
- If the error is from a partner organization (for example, a non-Microsoft cloud service provider), contact the partner to fix the issue.
...
- Find out which scenario applies to you, and make the necessary corrections. For example:
  - Confirm mail flow is working correctly and you didn't change the connector settings.
  - Check your on-premises email environment to see if the server is down.
  - Check for changes to your network infrastructure. For example, you changed internet service providers, so you now have different public IP addresses.
  - If the error is from a non-Microsoft cloud service provider, contact the service to fix the issue.

## Error code: 450 4.4.316 Connection refused

Microsoft 365 encountered a connection error when it tried to connect to the destination email server. A likely cause for this error is your firewall is blocking connections from Microsoft 365 IP addresses. Or, this error might be by design if you migrated your on-premises email system to Microsoft 365 and shut down your on-premises email environment.
...
- If no more messages should be delivered to your on-premises environment, select **Fix now** in the alert so Microsoft 365 can immediately reject the messages with invalid recipients. This action reduces the risk of exceeding your organization's quota for invalid recipients, which could affect normal message delivery. Or, use the following instructions to manually fix the issue:
...
- If the error is from a non-Microsoft service provider, contact the service to fix the issue.
...
- If network issues between your on-premises environment and Microsoft 365 are causing the problem, contact your network team to troubleshoot the issue.
- If the error is from a partner organization (for example, a non-Microsoft cloud service provider), contact the partner to fix the issue.
