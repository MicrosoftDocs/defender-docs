...
- In custom anti-spam policies and the default anti-spam policy, the available actions for spam filtering verdicts are described in the following table.
  ...
  ³ This value can be used as a condition only in [Mail flow rules in Exchange Server](/exchange/policy-and-compliance/mail-flow-rules/mail-flow-rules) to filter or route messages for mailboxes in on-premises Exchange environments. It is not supported in Exchange Online mail flow rules. 
