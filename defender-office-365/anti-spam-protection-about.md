...
- In custom anti-spam policies and the default anti-spam policy, the available actions for spam filtering verdicts are described in the following table.
  ...
  ³ You can this use value as a condition in [Mail flow rules in Exchange Server](https://learn.microsoft.com/en-us/exchange/policy-and-compliance/mail-flow-rules/mail-flow-rules) to filter or route messages for mailboxes in on-premises Exchange environments.
