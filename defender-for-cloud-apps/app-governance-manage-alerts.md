---
title: Remediate OAuth app threats with app governance alerts
ms.date: 07/03/2026
ms.topic: how-to
description: Investigate and manage app governance alerts in Microsoft Defender XDR to identify risky or malicious cloud apps and take remediation actions.
ms.reviewer: shragar
...
You can investigate alerts about malicious cloud apps and apps that may present risks to your organization in the Defender portal **Alerts** or **Incidents** pages.
...
By default, the Defender portal **Alerts** page lists new alerts generated by app governance based on threat detection rules and your active policies. View the details of a specific alert by selecting the alert. A page opens with additional information about the alert and options for managing the alert.

For example:

![Screenshot of the alert details pane showing the Alert story section with What happened and Recommended actions.](media/app-governance-detect-remediate-detect-threats/mapg-alert-story.png)

In the alert details page, you can get additional information from the **Alert story** section:
...
App policies that you configured for automatic remediation by using the **Action** setting in the app policy have a status of **Resolved**.
...
Based on app alert patterns, you can update the appropriate app policy and change its **Action** setting to perform automatic remediation. This removes your need to investigate and manually resolve future alerts that are generated by the app policy. For more information, see [Manage your app policies](app-governance-app-policies-create.md#manage-app-policies).

## Ban or approve an OAuth app connected to Salesforce and Google Workspace


> [!Note]
> The following app approval and ban procedures are only relevant for Salesforce and Google Workspace applications.
...
1. To ban the app, select the ban icon at the end of the app row in the table. For example:
    ...
    ![Screenshot of the Ban app action icon used to block an app in the app row.](media/ban-app-icon.png)
    ...
    ![Screenshot of the Ban app confirmation dialog with options to notify users before blocking the app.](media/ban-app.png)
...
1. To approve the app, select the approve icon at the end of the row in the table.
    ...
    ![Screenshot of the Approve app control for marking an app as allowed for connected app users.](media/approve-app.png)
...
> [!Note]
> The following notification and permission revocation procedures are only relevant for Salesforce and Google Workspace applications.
...
1. You can also revoke permissions to use the app for the user. Select the revoke icon at the end of the app row in the table, and then select **Revoke app**.

    ![Screenshot of the Revoke app action icon used to remove the app's permissions.](media/revoke-app.png)
