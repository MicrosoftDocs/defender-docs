...
- **Notifications:**
  ...
  - **User email notification** – Email messages can be customized and are sent to all violating file owners.
  - **Notify specific users** – Specific list of email addresses to receive these notifications.
...
- **Governance actions in apps** - Granular actions can be enforced per app, specific actions vary depending on app terminology.
  ...
  - **Change sharing**
    ...
    - **Expire shared link** - Ability to set an expiration date for a shared link after which it's no longer be active. (Box)
...
- **Notifications**
  - **Alerts** – Alerts can be triggered in the system and propagated via email, based on severity level.
  - **User email notification** – Email messages can be customized and are sent to all violating file owners.
  - **Notify additional users** – Specific list of email addresses to receive these notifications.
...
After creating multiple policies, a situation might arise in which the governance actions in multiple policies overlap. In this case, Defender for Cloud Apps processes the governance actions as follows:
...
|Accounts|File|Transfer all files ownership|On an account, you transfer one user's files to all be owned by a new person you select. The previous owner becomes an editor and can no longer change sharing settings. The new owner receives an email notification about the change of ownership.|Google Workspace|
...
|Activity policy, Accounts|Account|Require user to sign in again|Revokes all refresh tokens and session cookies issues to applications by the user. This action prevents access to any of the organization's data and forces the user to sign into all applications again.|Google Workspace, Office|
...
|File Policy|File|Notify file owner|Sends an email to the file owner, when a file violates a policy. In Dropbox, if no owner is associated with a file, the notification is sent to the specific user you set.|All apps|
...
|Files|File|Expire shared link|Set an expiration date for a shared link after which it's no longer active.|Box|
