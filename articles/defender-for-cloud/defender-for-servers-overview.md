...
- **Centralize management and reporting**: Defender for Cloud provides a single view of monitored resources, including machines protected by Defender for Servers. You can filter, sort, and cross-reference data to investigate and analyze resource security.
...
- **Benefit from agentless scanning**: Defender for Servers Plan 2 provides agentless machine scanning. Without an agent running on endpoints you can scan software inventory, assess machines for vulnerabilities, scan for machine secrets, and detect malware threats.
- **Protect against threats in near real-time**: Defender for Servers identifies and analyzes potential real-time threats by a number of means, and issues security alerts as needed.
- **Get intelligent threat detection**: Defender for Cloud evaluates events and detect threats using advanced security analytics and machine-learning technologies from multiple threat intelligence sources, including [Microsoft Security Response Center (MSRC)](https://www.microsoft.com/msrc). 
...
- **Unified alerts**: With the integration, alerts and vulnerability data from Defender for Endpoint appear in the Defender for Cloud dashboard in Azure. You can move to the Defender portal to drill down for detailed alert information and context.
...
**OS baseline misconfigurations** | Plan 2 | Defender for Cloud assesses and enforces security configurations using [built-in Azure policy initiatives](policy-reference.md), including its default [Microsoft Cloud Security Benchmark (MCSB)](/security/benchmark/azure/introduction) initiative.<br/><br/>Information is collected from machines using the [Azure Policy machine configuration](/azure/virtual-machines/extensions/guest-configuration). The older method that uses the MMA to collect information will be deprecated in November 2024. Support for assessing misconfigurations in Docker hub and Azure virtual machine scale sets will also be deprecated in November. <br/><br/> For OS recommendations based on [Linux](/azure/baselines/virtual-machines-linux-virtual-machines-security-baseline) and [Windows](/azure/baselines/virtual-machines-windows-virtual-machines-security-baseline) compute security baselines in the MCSB, Defender for Servers Plan 2 must be enabled.<br/><br/> Other MCSB recommendations in Defender for Cloud continue to be included in its free foundational posture management.
...
**File integrity monitoring** | Plan 2 only | [File integrity monitoring](file-integrity-monitoring-overview.md) examines files and registries for changes that might indicate an attack.<br/><br/> File integrity monitoring uses the Defender for Endpoint sensor to collect information. The previous collection method that used the MMA is now deprecated. [Learn more](migrate-file-integrity-monitoring.md).
