...
| Identity provider required for all Defender for Cloud components | **ASCDefendersOIDCIdentityProvider**                         | **Delete only if removing all Defender for Cloud components.** Use the AWS [IAM console](https://console.aws.amazon.com/iam/) or the [CLI](https://docs.aws.amazon.com/IAM/latest/UserGuide/iam_example_iam_DeleteOpenIdConnectProvider_section.html). You can retrieve a list the clients of the provider using the [AWS IAM API](https://docs.aws.amazon.com/IAM/latest/APIReference/API_GetOpenIDConnectProvider.html) | Shared component                    |
...
| Workload runtime threat protection  <br/><br/>  Kubernetes data plane hardening | Azure Arc enabled Kubernetes (Connects your EKS clusters to Azure) | You can remove Azure Arc-enabled Kubernetes per cluster via the [Azure CLI or Azure PowerShell](/azure/azure-arc/kubernetes/quickstart-connect-cluster?tabs=azure-cli#clean-up-resources)     Running this command deletes all Arc related resources       including extensions | Safe to remove      |
| Workload runtime threat protection  <br/><br/> Kubernetes data plane hardening              | Defender  sensor                                             | You can remove the Defender sensor per cluster using the [Azure portal, Azure CLI, or REST API](defender-for-containers-enable.md#remove-the-defender-sensor) | Safe to remove      |
| Kubernetes data plane hardening         | Azure Policy extension                                       |  You can remove Defender extensions per cluster using the [Azure portal, Azure CLI, or REST API](/azure/azure-arc/kubernetes/quickstart-connect-cluster?tabs=azure-cli#clean-up-resources) | Safe to remove      |
