...
Make sure you have a connector defined for the GitHub organization you are planning to use in Microsoft Defender for Cloud portal:
...
1. Go to Settings 
1. On the left panel select Secrets and Variables 
...
1. Click Access Keys under Settings. 
...
One way to do this is by connecting into the cluster and using the kubectl run command. Here is an example for AKS:
...
### Step 2: Create the 1st risk factor - Business Critical Rule 
...
1. In the Microsoft Defender for Cloud portal, go to Environment Settings 
1. Choose Resource Criticality
1. In the right pane, click the link to open Microsoft Defender.
1. Select Create a new classification.
1. Enter a name and description
1. Choose Cloud resource in the query builder. 
1. Write a query to set Resource Name equal to the name of the container you deployed to your cluster for validation
1. Click Next. 
1. In the Preview Assets page, if your resource has already been detected by Microsoft Defender, the name of the container should show with the asset type being K8-scontainer or K8s-pod. Even if it's not yet visible in the preview assets page, continue with the next step and Microsoft Defender will apply the criticality label to the container once detected. This can take 24hrs.
...
1. Test the repo is been picked up by GitHub agentless scanning. 

Go to Cloud Security Explorer and perform the following query:

1. Validate the container image has been scanned by the MDC (in ACR ) and was used to create a container.  In your query, add the conditions below.

1. Validate that the container is running and the AKS cluster been scanned by MDC.
...
> [!NOTE]
> After your resource has been classified as critical, it can take up to 12hrs for it to be reflected in GitHub. 

1. In GitHub, go to the GitHub organization you used for the setup testing
1. Click Security, 
1. Select Campaigns
1. Click Create a campaign from code scanning filters. 
1. Create the Campaign below. This will show open alerts with medium severity where the image deployed from the repository is tied to a critical resource. Your test repository should be detected with this campaign.
1. Click Save and then Publish as campaign. 
...
1. Click the Associated CVEs tab. Notice that some CVEs have a link in the column Related GitHub Alerts. 
1. Clicking on View on GitHub link would open the relevant GHAS security alert.

#### Engineering mobilization 

To close the loop between Security & Engineering teams you can create a GitHub issue for a containerized application prioritizing to the engineering team the security issues on which they should be focusing, including passing findings that were not picked up by GHAS but were detected by MDC for CVEs that are NOT part of direct dependencies (e.g., vulnerabilities in the base image, OS, or third-party software like NGNIX). 

The GitHub issue would be auto-generated with all the CVEs found in the scope of the recommendation, the one with and without Dependabot alerts matches including additional runtime context on the repo of origin.
...
1. If seems reasonable, apply the fix
