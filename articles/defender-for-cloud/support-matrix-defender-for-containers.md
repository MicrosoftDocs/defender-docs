...
| Attack path analysis | A graph-based algorithm that scans the cloud security graph. The scans  expose exploitable paths that attackers might use to breach your  environment. | ACR, AKS | GA | GA | Activated with plan | Defender CSPM (requires Agentless discovery for Kubernetes to be enabled) | AWS |
