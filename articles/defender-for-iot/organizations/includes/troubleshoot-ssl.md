...
|**The CRL (Certificate Revocation List) location is not reachable. Verify the URL can be accessed from this appliance** | Make sure that your network configuration allows the sensor to reach the CRL server defined in the certificate. <br> For more information, see [Verify CRL server access](../ot-deploy/create-ssl-certificates.md#verify-crl-server-access). |
