...
- **Workflow without behaviors:** An analyst triaging an incident might have to pivot across multiple raw log tables to piece together a timeline. 

  *Example*: An alert triggers on a suspicious AWS activity. The analyst has to query `AWSCloudTrail` logs, and then might have to query firewall logs to understand what the user or host did around that time. This is time-consuming and requires familiarity with each data source’s schema.

- **Workflow with behaviors:** Sentinel automatically stitches relevant events into behavior entries that can be attached to the incident or queried on demand. 

  *Example:* An alert flags possible credential exfiltration. In the `BehaviorInfo` table, the analyst sees the behavior **"Suspicious mass secret access via AWS IAM by User123"** mapped to **MITRE Technique T1552 (Unsecured Credentials)**. This single behavior was generated by aggregating 20 AWS log entries. The analyst immediately knows that User123 accessed many secrets – crucial context to escalate the incident – without manually reading all 20 log entries.
...
- **Workflow without behaviors:** Hunting in Sentinel across multiple datasets means writing complex queries and joining different tables or scanning specific event IDs. Each log source has its own format, so hunters have to normalize data mentally or using KQL, and often miss subtle sequences. 

  *Example:* Hunting for signs of reconnaissance might require checking an `AWSCloudTrail` event *and* certain firewall connection patterns separately. Security context is mainly available for alerts and incidents, not for raw telemetry, making proactive hunts harder.

- **Workflow with behaviors:** Behaviors are *already normalized and enriched* with context. A hunter can query the `BehaviorInfo` table by MITRE technique or by high-level `Title` across sources. For instance, they can search for any behaviors mapped to `Initial Access` tactics in the last 24 hours, regardless of whether they came from AWS or on-premises logs. Behaviors distill what happened in a source-agnostic way, so the hunter can find relevant activity without writing source-specific queries. This improves hunting efficacy and helps detect attacks earlier in the kill chain, even if an alert isn`t generated.

  The `BehaviorInfo` table includes columns like `Categories` (MITRE tactics) and `MitreTechniques`, and a descriptive `Title`. A hunter can filter on these columns - for example: 
  ...
  Hunters can also see the rarer behaviors in the environment, using `count distinct` on the `Title` field, pivot to the interesting behaviors type, see which entities are involved, and then investigate further. Behaviors link back to underlying data - in the `BehaviorId` and `AdditionalFields` columns, which often contain original log references - so a hunter can always drill down to raw logs for verification if needed.

  *Example:* A threat hunter wants to find stealthy credential access. They query for behaviors with `Title` containing "enumerate credentials". Sentinel behaviors returns a few instances of **"Attempted credential dump from Vault by user AdminJoe"** (a behavior derived from CyberArk logs). Even though alerts weren't fired, this behavior stands out. The hunter investigates and finds it was an unusual activity for AdminJoe, potentially catching an early-stage attack. Without Sentinel behaviors, this pattern might be lost in thousands of Vault audit events.

  Hunters can also use behaviors to hunt **by MITRE tactic**. For example, filter `BehaviorInfo` where `Categories == "Lateral Movement"` to instantly see all lateral movement behaviors across any source. This is powerful because it abstracts *how* an action was done – focusing instead on the intent (tactic).
...
- **Workflow without behaviors:** Creating custom correlation rules on raw logs can be challenging. Each data source has a different schema, so writing a generalized detection - for example, "Multiple failed admin actions across cloud and on-prem" - means either writing multiple rules or a very complex single query with normalization logic. Lacking context, many rules have to include lots of conditions to reduce false positives, or they rely on alerts as inputs rather than raw events. Automation (SOAR playbooks) can also be challenging if triggers are too low-level – you risk responding to benign events.

- **Workflow with behaviors:** Behaviors provide high-quality, normalized signals that can serve as the triggers or building blocks for detection rules. Behaviors already aggregate related events; therefore, a single behavior can replace what previously required correlating many atomic events. Each behavior is also mapped to MITRE tags and entity roles, which you can use in rule logic - for example, a rule can look for a sequence consisting of a **T1078 (Valid Accounts)** behavior followed by a **T1566 (Phishing)** behavior by the same user. Detection engineers can write rules that are **simpler** and more **explainable**, because the rule conditions align with human-understandable behavior descriptions.

  You can use the `BehaviorInfo` table in analytic rule queries just like any other log. 
...
Behaviors can also act as reliable triggers for automation. Instead of creating alerts for non-risky activities, use behaviors as building blocks for automation - for example, to send an email or verify with a user.
