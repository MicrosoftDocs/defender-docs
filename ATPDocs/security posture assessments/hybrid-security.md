...
|(Replace Enterprise or Domain Admin account for Microsoft Entra Connect AD DS Connector account)[##remediation-for-replace-enterprise-or-domain-admin-account-for-entra-connect-ad-ds-connector-account]     | Smart attackers often target Microsoft Entra Connect in on-premises environments due to the elevated privileges associated with its AD DS Connector account (typically created in Active Directory with the MSOL_ prefix). Using an **Enterprise Admin** or **Domain Admin** account for this purpose significantly increases the attack surface, as these accounts have broad control over the directory.
