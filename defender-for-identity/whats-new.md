...
- **Identity Security dashboard**: The Defender for Identity dashboard has been redesigned as the **Identity Security dashboard**, providing a unified view of your identity security posture for all identity sources. The dashboard now includes rotating summary cards for identity providers, on-premises identities, SaaS identities, PAM & IGA integrations, and non-human identities—along with widgets for deployment status, highly privileged identities, users at risk, and domains with unsecured configurations. For more information, see [The Identity Security dashboard](dashboard.md).

- **Coverage and maturity page**: A new **Coverage and maturity** page helps you track your organization's identity security coverage for identity providers, on-premises identities, SaaS identities, and PAM & IGA integrations. Each source displays a maturity level—Connected, Protected, Fortified, or Resilient—along with identity counts, coverage scores, and prioritized setup tasks to improve your posture. For more information, see [Coverage and maturity](../defender-xdr/identity-security/coverage-maturity.md).

- **Identity inventory redesign**: The Identity inventory page now provides a unified view of human and non-human identities with dedicated tabs for each. The page also includes new insight cards for classifying critical assets, viewing highly privileged identities, identifying critical Active Directory service accounts, and accessing cloud application accounts. For more information, see [View the Identity inventory](identity-inventory.md).

- **Non-human identities (Preview)**: You can now view and investigate non-human identities—including Entra ID apps, Active Directory service accounts, Google Workspace apps, and Salesforce apps. A new **Non-human identities** tab on the Identity inventory page provides a consolidated view with statistics for risky, highly privileged, overprivileged, unused, and externally published identities, and a dedicated investigation page lets you drill into details for each identity. For more information, see [Identity inventory](identity-inventory.md) and [Investigate non-human identities](../defender-xdr/investigate-non-human-identities.md).

- **Risk score tab on the identity page**: A new **Risk score** tab on the identity investigation page shows a 0–100 risk score with percentile comparison, likelihood of compromise broken down by MITRE ATT&CK kill chain stage, and impact of compromise based on criticality and PIM role assignments. You can also view risk trends and reset the risk score. For more information, see [Investigate an identity](../defender-xdr/investigate-users.md).

- **Identity security recommendations**: Identity security recommendations are now consolidated from all identity sources into a single view. View recommendations for Active Directory, Microsoft Entra ID, SaaS applications (including Microsoft, Atlassian, GitHub, Google Workspace, Salesforce, ServiceNow, and more), and supported non-Microsoft identity providers (Okta, PingOne, CyberArk, and SailPoint). For more information, see [Identity security recommendations](../defender-xdr/identity-security/identity-security-recommendations.md).

- **Domain investigation page**: A new domain investigation page provides a comprehensive view of Active Directory domain security. The page includes domain properties, deployment health, identity summary, service account breakdown, sensitive entities, active recommendations, group policies, and trust relationships. For more information, see [Investigate a domain](investigate-domain.md).

- **Password protection page**: A new **Password protection** page consolidates identity password risk from Active Directory, Microsoft Entra ID, and Okta. The page includes tabs for password hygiene, password policies, leaked credentials, and exposed passwords, helping you identify accounts with weak or compromised credentials. For more information, see [Password protection](password-protection.md).

### Defender for Identity sensor updates

- **Migrate Defender for Identity sensors from v2.x to v3.x**: You can now migrate Defender for Identity sensors from v2.x to v3.x directly from the Microsoft Defender portal. The v2.x sensor continues running during the migration until the v3.x sensor is ready, so there's no downtime. Eligible servers appear as **Ready for migration** on the **Sensors** page, and migration takes up to 20 minutes. For more information, see [Migrate to Defender for Identity sensor v3.x](deploy/migrate-to-sensor-v3.md).

- **Sensor v3.x support for domain controllers with additional identity roles**: Defender for Identity sensor v3.x now supports domain controllers that also run AD FS, AD CS, or Entra Connect identity roles. Detections and identity security posture management (ISPM) recommendations for these roles will roll out gradually, starting with Entra Connect. For v3, these domain controllers require Windows Server 2019 or later with at least the March 2026 Cumulative Update. For more information, see [Defender for Identity deployment overview](deploy/deploy-defender-identity.md) and [Sensor v3.x prerequisites](deploy/prerequisites-sensor-version-3.md).
