...
- **LocalSystem impersonation on the domain controller**: The Defender for Identity sensor runs actions by impersonating the LocalSystem account on the domain controller. This behavior can be modified by configuring group Managed Service Account (gMSA) scoping and permissions.

- **Microsoft Entra ID–based execution**: For some remediation actions, Defender for Identity creates an enterprise application in Microsoft Entra ID. The enterprise application, Microsoft Defender for Identity, is created automatically in the tenant and is used only to execute remediation actions. When a user initiates an action from the Defender portal, the request is authorized based on the user’s Entra ID roles and executed by the Defender for Identity application, enforcing Entra ID role‑based access control (RBAC) and audit logging.
