...
As organizations increasingly adopt hybrid and multi-cloud environments, securing identities across platforms becomes critical. Connecting SailPoint Identity Security Cloud with Microsoft Defender for Identity (MDI) gives you the ability to detect, investigate, and respond to identity-based threats across both cloud and on-premises infrastructures. 
...
|------------|------------|
| View SailPoint accounts in the identity inventory | - Adds SailPoint Identity users into the identity inventory and correlates them with identities from on-premises, Active Directory and Microsoft Entra ID.  |
| Improve SailPoint security posture | Evaluates SailPoint Identity accounts for security risks such as stale privileged accounts and excessive privileged role assignments, and generates posture recommendations. Example recommendations include: <br> - Change password for SailPoint Identity privileged user accounts<br>- Remove stale SailPoint Identity privileged accounts <br>- Limit the number of SailPoint Identity accounts with system admin role <br>- High number of SailPoint Identity accounts with a privileged role assigned |
