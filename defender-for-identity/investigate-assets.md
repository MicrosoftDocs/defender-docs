...
|[Remediation actions](/microsoft-365/security/defender/investigate-users#remediation-actions)      |     Respond to compromised identities by disabling their accounts or resetting their password. After taking action, you can check on the activity details in the Microsoft Defender XDR **Action center**. You can perform response actions such as disabling a user, resetting a password for Active Directory accounts, or revoking sessions. These actions can be applied at the Identity level (all linked accounts) or at the Account level (a single account or a selected set of accounts). |
