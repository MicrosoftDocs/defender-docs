...
|---------|---------|
|[Overview tab](/microsoft-365/security/defender/investigate-users#overview)       | General identity data, such as the Microsoft Entra identity risk level, the number of devices the identity is signed in to, when the identity was first and last seen, the identity's accounts and more important information.  <br><br>Use the **Overview** tab to also view graphs for incidents and alerts, and an organizational tree, entity tags.       |
...
|[Remediation actions](/microsoft-365/security/defender/investigate-users#remediation-actions)      |     Respond to compromised identities by disabling their accounts or resetting their password. After taking action, you can check on the activity details in the Microsoft Defender XDR **Action center**.|
