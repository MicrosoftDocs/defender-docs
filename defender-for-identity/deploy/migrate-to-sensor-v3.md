...
- [Understand DSA and gMSA health alerts in environments with both v2 and v3 sensors](deploy-sensor-v3.md#dsa-and-gmsa-health-alerts-in-environments-with-both-v2-and-v3-sensors). If your workspace still has a Directory Service Account (DSA) or group Managed Service Account (gMSA) configured for v2 sensors, the **Directory services user credentials are incorrect** health alert continues to fire on all sensors, including v3 sensors. This is by design. V3 sensors ignore the DSA and gMSA for auditing and response actions, but the health alert validates credentials at the workspace level. To stop the alert, remove the DSA or gMSA after all sensors are migrated to v3.
