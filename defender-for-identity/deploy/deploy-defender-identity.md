...
> [!IMPORTANT]
> In mixed environments, select **Automatically use the sensor's local system account** for all sensors. The v3.x sensors use the local system account regardless of gMSA configuration, and any gMSA accounts configured for v2.x sensors are ignored by v3.x sensors. For more information, see [Sensor v3.x service account requirements](deploy-sensor-v3.md#service-account-requirements).
